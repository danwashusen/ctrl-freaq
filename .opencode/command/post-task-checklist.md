---
name: Post-task quality checklist
description:
  Ensure a task is integration-ready by following a quality checklist after
  implementation.
---
...
Ingest the entire playbook, then follow the steps in the 'Process' section until
...
3. Ensure the `pnpm lint:fix` command succeeds, fix any linting errors and
   warnings.
   - Prefer re-writing code over adding suppressions.
4. Ensure the `pnpm typecheck` command succeeds, fix any errors and warnings.
5. Ensure the `pnpm test` command succeeds, analyze the failures and suggest
   fixes.
