...
1. Load Spec Kit configuration and gather paths:
   - Load `/.specify.yaml` if present; otherwise use
     `.specify/config-default.yaml`, extracting the root `spec-kit` entry as
     `SPEC_KIT_CONFIG`.
   - Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` from
     repo root **once** (combined `--json --paths-only` / `-Json -PathsOnly`)
     and parse:
     - `FEATURE_DIR`
     - `FEATURE_SPEC`
     - (Optionally capture `IMPL_PLAN`, `TASKS` for future chained flows.)
   - If JSON parsing fails, abort and instruct the user to re-run
     `/speckit.specify` or verify the feature branch setup.
...
2. Load supporting context, then run the ambiguity scan:
   ...
   - Read the constitution from `SPEC_KIT_CONFIG.constitution.path` and note any
     principles that require clarifications.
   - If defined, resolve each `SPEC_KIT_CONFIG.clarify.documents[*].path`
     (fallback to `SPEC_KIT_CONFIG.plan.documents` if `clarify` is absent), read
     the files, and keep their `context` handy when drafting questions. Treat
     all referenced files as read-only and note missing files without failing.
   - Read the changelog at `SPEC_KIT_CONFIG.changelog.path` (if present) to
     preserve established decisions.
   - If the spec lacks a `## Clarifications` section or prior sessions,
     emphasize criticality of capturing clarifications now before
     `/speckit.plan` proceeds.
   - Perform a structured ambiguity & coverage scan using this taxonomy. For
     each category, mark status: Clear / Partial / Missing. Produce an internal
     coverage map used for prioritization (do not output raw map unless no
