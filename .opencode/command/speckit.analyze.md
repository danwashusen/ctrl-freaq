...
- Resolve each `SPEC_KIT_CONFIG.analyze.documents[*].path` relative to the repo
  root, read the file contents, and treat the provided `context` as guidance.
  Note missing files without failing the command.
- Read the changelog from `SPEC_KIT_CONFIG.changelog.path` (if present) to
  incorporate historical context.
- Read the constitution from `SPEC_KIT_CONFIG.constitution.path` for principle
  validation. Treat all referenced files as read-only.
...
- Load the constitution referenced by `SPEC_KIT_CONFIG.constitution.path` for
...
- Use insights from `SPEC_KIT_CONFIG.analyze.documents` to surface domain
  definitions missing from spec/plan/tasks and recommend updates.
...
- Conflicting requirements (e.g., one requires Next.js while other specifies
  Vue)
- When supplementary documents capture mandatory constraints not reflected in
  the core artifacts, flag them as underspecification/inconsistency and direct
  updates rather than assuming downstream commands will see those documents.
