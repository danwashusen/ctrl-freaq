...
export const DocumentEditor = memo<DocumentEditorProps>(({ documentId, className }) => {
  ...
  const { toc, setTableOfContents } = useDocumentStore(state => ({
    toc: state.toc,
  ...
  const client = useMemo(() => createSectionEditorClient(), []);
  ...
  const {
    ...
    api: {
      saveDraft: payload => client.saveDraft(payload.sectionId, payload),
      checkConflicts: payload => client.checkConflicts(payload.sectionId, payload),
      fetchDiff: payload => client.getDiff(payload.sectionId),
      listConflictLogs: payload => client.listConflictLogs(payload.sectionId),
    },
  ...
  useEffect(() => {
    if (activeSection) {
      updateSection({ id: activeSection.id, viewState: isEditing ? 'edit_mode' : 'read_mode' });
    }
  ...
  useEffect(() => {
    ...
    setDraftMetadata(activeSection.id, {
      draftId: sectionDraftState.draftId ?? null,
      draftVersion: sectionDraftState.draftVersion ?? null,
      draftBaseVersion: sectionDraftState.draftBaseVersion ?? null,
      latestApprovedVersion: sectionDraftState.latestApprovedVersion ?? null,
    });
  }, [
    activeSection,
  ...
  useEffect(() => {
    ...
    setConflictState(activeSection.id, {
      ...
      conflictReason: sectionDraftState.conflictReason ?? null,
      latestApprovedVersion: sectionDraftState.latestApprovedVersion ?? null,
    });

    if (draftState.conflictState === 'rebase_required' || draftState.conflictState === 'blocked') {
  ...
  useEffect(() => {
    ...
    recordManualSave(activeSection.id, {
      lastSavedAt: sectionDraftState.lastSavedAt ?? null,
      lastSavedBy: sectionDraftState.lastSavedBy ?? null,
      lastManualSaveAt: sectionDraftState.lastManualSaveAt ?? null,
      summaryNote: draftState.summaryNote ?? null,
    });
  }, [
    activeSection,
  ...
  }, [documentId, sectionsList, activeSectionId, setTableOfContents]);
  ...
  const handleSubmitReview = useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unable to submit draft for review.';
                ...
                {isEditing && (
                  <div className="mt-6 space-y-6" data-testid="section-editor-panel">
                    <FormattingToolbar
                      ...
                      onToggleBulletList={() => updateDraft(`${draftState.content}\n- Bullet item`)}
                      onInsertTable={() =>
                        updateDraft(`${draftState.content}\n| Col A | Col B |\n| --- | --- |`)
                      }
                      onInsertLink={() =>
