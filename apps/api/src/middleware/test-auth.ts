...
/**
 * Deterministic simple-auth user IDs consumed by repository tests and fixtures.
 */
export const DEFAULT_TEST_USER_ID = 'user-local-author';
export const TEMPLATE_MANAGER_USER_ID = 'template_manager';
  ...
  if (isTestRuntime()) {
    ...
    if (auth && typeof auth === 'string' && auth.toLowerCase().startsWith('bearer')) {
      ...
      if (token === MOCK_JWT_TOKEN) {
        ...
        req.auth.sessionId = req.auth.sessionId ?? `simple:${DEFAULT_TEST_USER_ID}`;
        ...
        req.auth.sessionId = req.auth.sessionId ?? `simple:${TEMPLATE_MANAGER_USER_ID}`;
      }
    }
  }
  next();
