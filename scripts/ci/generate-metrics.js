...
function calculatePerformanceScore(jobResults, fileSystemMetrics) {
  ...
  const sizeEfficiency = Math.max(
    ...
    Math.min(
      ...
      Math.round(
        20 *
          (reasonableProjectSize / Math.max(fileSystemMetrics.totalSizeMB, reasonableProjectSize))
      )
    )
  );
  ...
  const hasGoodStructure =
    fileSystemMetrics.fileTypes.ts > 10 &&
...
function generateHumanReport(metrics) {
  ...
  if (metrics.project) {
    ...
    lines.push(
      `Workspaces: ${metrics.project.workspaces.total} (${metrics.project.workspaces.apps} apps, ${metrics.project.workspaces.packages} packages)`
    );
