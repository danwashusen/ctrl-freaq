...
11. Assemble review deliverables
    - Populate `.specify/templates/audit-template.md` exactly as structured; the
      template is the canonical output format.
...
14. **MANDATORY** Archival write-back and task generation
    ...
    - Review output filing: present the rendered report to the operator, and
      create or amend `audit.md` inside `FEATURE_DIR` with the contents of the
      report.
    - Open `tasks.md` in `FEATURE_DIR` and overwrite or create a **Phase 4.R:
      ...
      - Each finding documented in the report must be represented as a new
        ...
        Reference `audit.md` (or the specific evidence path) in the description
...
15. Update the changelog
    ...
    - Read the feature spec captured in Step 3 and use its main heading (e.g.,
      `# Feature Specification: New Section Content Flow`) to derive a concise
      changelog entry title by removing `Feature Specification:` (or similar
      prefixes) and trimming whitespace (e.g., `New Section Content Flow`).
    - Ensure the changelog contains **one entry per feature identifier/branch**:
      search for an existing `##` heading that includes the feature identifier
      (e.g., `009-epic-2-story-4`) or the derived title, update that section in
      place (date stamp, summary, highlights) when it exists, and only create a
      new section when no matching entry is present.
    - Create or update a section dedicated to this review that captures the
      ...
      the entry with the feature identifier or branch name and the derived title
      for quick recognition by future readers.
    ...
    - Include a short "Technical Highlights" list that spotlights the most
      important implementation assets—new or significantly modified files,
      directories, migrations, scripts, or configuration touchpoints—using
      absolute or repository-relative paths so a developer can jump directly
    ...
    - Note any follow-up work that arises from those code changes (migrations,
      deprecations, roll-out steps) and link back to `audit.md` for detailed
