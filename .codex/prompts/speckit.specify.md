...
2. Run the script `.specify/scripts/bash/create-new-feature.sh --json "$ARGUMENTS"` from repo root and parse its JSON output for `BRANCH_NAME` and `SPEC_FILE`. All file paths must be absolute. The script now:
...
- Reference files using repository-root anchored paths (e.g., `/frontend/src/...`) and avoid host-specific paths.
